...
for folder in ['/A', '/B']:
    for subfolder, subset in zip(subfolders, [subset_train, subset_val, subset_test]):
        try:
            os.makedirs(path + folder + subfolder, exist_ok=True)
        except Exception as e:
            print(f"Error creating directory {path + folder + subfolder}: {e}")
            continue
...
if augment:
    ...
    for image in [item for item in os.listdir(os.path.join(path, augmentor_subfolder)) if not os.path.isdir(os.path.join(path, augmentor_subfolder, item))]:
        ...
        #     dest_path = os.path.join(path, 'A/train', image_new)
        # else:
        #     image_new = image_new.replace('train_original_', '').replace('.bmp', '_augmented.bmp')
        #     dest_path = os.path.join(path, 'B/train', image_new)
        
        if 'groundtruth' in image_new:
            image_new = image_new.replace('_groundtruth_(1)_train_', '').replace('.png', '_augmented.png')
            dest_path = os.path.join(path, 'A/train', image_new)
        else:
            image_new = image_new.replace('train_original_', '').replace('.png', '_augmented.png')
            dest_path = os.path.join(path, 'B/train', image_new)

        try:
            shutil.copy(os.path.join(path, augmentor_subfolder, image), dest_path)
        except FileNotFoundError:
            print(f"Warning: File not found: {os.path.join(path, augmentor_subfolder, image)}")
        except Exception as e:
            print(f"Error copying file {image}: {e}")

    # Remove the output folder
    try:
        shutil.rmtree(os.path.join(path, 'B/train/output'))
    except Exception as e:
